...
var GitlabIssuesSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Gitlab instance URL").setDesc("Use your own Gitlab instance instead of the public hosted Gitlab.").addText((text) => text.setPlaceholder("https://gitlab.com").setValue(this.plugin.settings.gitlabUrl).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.gitlabUrl = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Personal Access Token").setDesc("Create a personal access token in your Gitlab account and enter it here.").addText((text) => text.setPlaceholder("Token").setValue(this.plugin.settings.gitlabToken).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.gitlabToken = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Template File").setDesc("Path to an Obsidian note to use as the template.").addText((text) => text.setPlaceholder("your-template-file.md").setValue(this.plugin.settings.templateFile).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.templateFile = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Output Folder").setDesc("Path to an Obsidian folder to write output files to.").addText((text) => text.setPlaceholder("/Gitlab Issues/").setValue((0, import_obsidian.normalizePath)(this.plugin.settings.outputDir)).onChange((value) => __async(this, null, function* () {
      value = (0, import_obsidian.normalizePath)(value);
      this.plugin.settings.outputDir = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Issues Filter").setDesc("The query string used to filter the issues.").addText((text) => text.setPlaceholder("due_date=month").setValue(this.plugin.settings.filter).onChange((value) => __async(this, null, function* () {
      this.plugin.settings.filter = value;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Refresh Rate").setDesc("That rate at which gitlab issues will be pulled.").addDropdown((value) => value.addOptions({ off: "off", "15": "15", "30": "30", "45": "45", "60": "60", "120": "120" }).setValue(this.plugin.settings.intervalOfRefresh).onChange((value2) => __async(this, null, function* () {
      this.plugin.settings.intervalOfRefresh = value2;
      this.plugin.scheduleAutomaticRefresh();
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("GitLab Level").addDropdown((value) => value.addOptions({ personal: "Personal", project: "Project", group: "Group" }).setValue(this.plugin.settings.gitlabIssuesLevel).onChange((value2) => __async(this, null, function* () {
      this.plugin.settings.gitlabIssuesLevel = value2;
      yield this.plugin.saveSettings();
      this.display();
    })));
    if (this.plugin.settings.gitlabIssuesLevel !== "personal") {
      const gitlabIssuesLevelIdObject = this.plugin.settings.gitlabIssuesLevel === "group" ? { text: "Group", url: "https://docs.gitlab.com/ee/user/group/#get-the-group-id" } : { text: "Project", url: "https://docs.gitlab.com/ee/user/project/working_with_projects.html#access-the-project-overview-page-by-using-the-project-id" };
      ...
      new import_obsidian.Setting(containerEl).setName(`Set Gitlab ${gitlabIssuesLevelIdObject.text} Id`).setDesc(descriptionDocumentFragment).addText((value) => value.setValue(this.plugin.settings.gitlabAppId).onChange((value2) => __async(this, null, function* () {
        this.plugin.settings.gitlabAppId = value2;
        yield this.plugin.saveSettings();
      })));
    }
    new import_obsidian.Setting(containerEl).setName("Purge issues that are no longer in Gitlab?").addToggle((value) => value.setValue(this.plugin.settings.purgeIssues).onChange((value2) => __async(this, null, function* () {
      this.plugin.settings.purgeIssues = value2;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Show refresh Gitlab issues icon in left ribbon?").addToggle((value) => value.setValue(this.plugin.settings.showIcon).onChange((value2) => __async(this, null, function* () {
      this.plugin.settings.showIcon = value2;
      yield this.plugin.saveSettings();
    })));
    new import_obsidian.Setting(containerEl).setName("Should refresh Gitlab issues on Startup?").addToggle((value) => value.setValue(this.plugin.settings.refreshOnStartup).onChange((value2) => __async(this, null, function* () {
      this.plugin.settings.refreshOnStartup = value2;
      yield this.plugin.saveSettings();
    })));
...
// src/logger.ts
function logger_default(message) {
...
// src/default-template.js
var default_template_default = `---
id: {{id}}
title: {{{title}}}
dueDate: {{due_date}}
webUrl: {{web_url}}
...
var Filesystem = class {
  ...
  createOutputDirectory() {
    this.vault.createFolder(this.settings.outputDir).catch((error) => {
      if (error.message !== "Folder already exists.") {
        logger_default("Could not create output directory");
      }
    });
  }
  purgeExistingIssues() {
    const outputDir = this.vault.getAbstractFileByPath(this.settings.outputDir);
    if (outputDir instanceof import_obsidian2.TFolder) {
      import_obsidian2.Vault.recurseChildren(outputDir, (existingFile) => {
        if (existingFile instanceof import_obsidian2.TFile) {
          this.vault.delete(existingFile).catch((error) => logger_default(error.message));
        }
      });
    }
  }
  processIssues(issues) {
    this.vault.adapter.read(this.settings.templateFile).then((rawTemplate) => {
      ...
      issues.map((issue) => this.writeFile(issue, (0, import_handlebars.compile)(default_template_default.toString())));
    });
  }
  writeFile(issue, template) {
    this.vault.create(this.buildFileName(issue), template(issue)).catch((error) => logger_default(error.message));
  }
  buildFileName(issue) {
...
// src/gitlab-loader.ts
var GitlabLoader = class {
  constructor(app2, settings) {
    this.fs = new Filesystem(app2.vault, settings);
    this.settings = settings;
  }
  getUrl() {
...
var GitlabIssuesPlugin = class extends import_obsidian4.Plugin {
  ...
  onload() {
    return __async(this, null, function* () {
      logger_default("Starting plugin");
  ...
  addIconToLeftRibbon() {
    if (this.settings.showIcon) {
      if (!this.iconAdded) {
        (0, import_obsidian4.addIcon)("gitlab", gitlab_icon_default);
        this.addRibbonIcon("gitlab", "Gitlab Issues", (evt) => {
          this.fetchFromGitlab();
        });
        this.iconAdded = true;
      }
  ...
  fetchFromGitlab() {
    new import_obsidian4.Notice("Updating issues from Gitlab");
