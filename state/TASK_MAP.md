# TASK_MAP

## Git-only (G-*)

| ID | Task | Status |
|----|------|--------|
| G1–G5 | Plan, schema, template, #5, Sera wording | **Done** |
| G6 | (선택) Open Q #4·#6·#7 초안 | **Pending** |

## Domain (P-*)

| ID | Task | Status |
|----|------|--------|
| P1 | Domain inventory | **Done** |
| P2 | excelion + excelion-forge state seed | **Done** |
| P3 | forge vs excelion-forge Decision | **Done** — **D20** |

## Owner local (L-*)

| ID | Task | Status |
|----|------|--------|
| L-1…L-5 | Ollama + Cline | **Done** |
<<<<<<< HEAD
| L-3 | .clinerules 배치 | **Done** |
| L-4 | Cline 작업 루프 검증 | **Done** |
| L-5 | Cline 보완 마무리 점검 | **Done** |
| L-6 | untrack / gitignore 정리 | **Done** |
| L-7 | rebase (impl/atlas-extension) | **Done** |
| L-8…L-10 | merge, tag, PR | **Pending** |
=======
| L-6 | untrack / gitignore cleanup | **Done** |
| L-7 | impl/atlas-extension rebase | **Done** (6507877 / main 7a47a70) |
| L-8 | merge / tag / PR (선택) | **Pending** |
| L-9…L-10 | (미정) | **Pending** |
>>>>>>> 56a629f (chore(state): TASK_MAP L-6/L-7 Done 정리, L-8+ Pending 분리)

## Implementation

| ID | Task | Status |
|----|------|--------|
| IMP-1 | Extension | Open — issue #2 |

### Decision

- D20: Canonical Forge = `projects/excelion-forge/` only.
- D19: SERA project abandoned.

## 검증
1. cat state/TASK_MAP.md
2. L-6…L-10 중복 행이 없어야 함
3. L-6·L-7 = Done, L-8+ = Pending