...
func apply_damage(amount: float) -> void:
	...
	var armor := data.armor if data != null else 0.0
	current_hp = maxf(0.0, current_hp - maxf(0.0, amount - armor))
	if current_hp <= 0.0:
		_die()


func apply_slow(next_factor: float, duration: float) -> void:
	if is_dead:
		return
	slow_factor = clampf(next_factor, 0.2, 1.0)
...
func _physics_process(delta: float) -> void:
	...
	global_position = global_position.move_toward(target_position, speed * slow_factor * delta)
