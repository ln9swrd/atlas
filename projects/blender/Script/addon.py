...
def resolve_bind_host(explicit_host=None):
    """Resolve a bind host that is reachable from other machines.

    We intentionally avoid localhost/127.0.0.1 for the default case because
    those addresses only accept connections from the local machine.
    """
    env_candidates = [
        os.getenv("BLENDER_MCP_HOST"),
        os.getenv("BLENDER_HOST"),
        os.getenv("BLENDER_MCP_BIND_HOST"),
    ]
    ...
    if explicit_host in {None, "", "localhost", "127.0.0.1"}:
        return "0.0.0.0"
...
class BLENDERMCP_OT_StartServer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Always create a fresh server instance so an older localhost-bound
        # server is not reused after a reload.
        if hasattr(bpy.types, "blendermcp_server") and bpy.types.blendermcp_server:
            bpy.types.blendermcp_server.stop()
            del bpy.types.blendermcp_server

        bpy.types.blendermcp_server = BlenderMCPServer(host=resolve_bind_host(), port=scene.blendermcp_port)
...
def register():
    ...
    if auto_start:
        if hasattr(bpy.types, "blendermcp_server") and bpy.types.blendermcp_server:
            bpy.types.blendermcp_server.stop()
            del bpy.types.blendermcp_server
        bpy.types.blendermcp_server = BlenderMCPServer(host=resolve_bind_host(), port=port)
