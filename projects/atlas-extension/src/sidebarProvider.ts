...
export class AtlasSidebarProvider implements vscode.WebviewViewProvider {
  ...
  private _ollamaHost = 'http://192.168.219.254:11434';
  ...
  private async _handleUserPrompt(userPrompt: string) {
    ...
    const ollamaHost = config.get<string>('ollamaHost') || this._ollamaHost;
    const preferredModel = config.get<string>('model') || 'qwen3:14b';
...
`;
    ...
    const child = spawn(pythonExecutable, ['-u', scriptPath, userPrompt], {
      cwd: orchestratorCwd,
      env: {
        ...process.env,
  ...
  private _executeCliCommand(command: string) {
    const workspaceFolders = vscode.workspace.workspaceFolders;
