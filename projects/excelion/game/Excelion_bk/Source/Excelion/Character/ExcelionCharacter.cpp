...
AExcelionCharacter::AExcelionCharacter()
{
    ...
    // Default fallback cube mesh for editor preview
    ...
    // ---
    // Assign Manny skeletal mesh as default character mesh.
    // This ensures the character is rendered with the intended Manny model
    // when spawned, even if the blueprint overrides are missing.
    // NOTE: The asset path is hard‑coded because the Manny asset is part of the
    // project's content and is guaranteed to exist.
    static ConstructorHelpers::FObjectFinder<USkeletalMesh> CharacterMesh(TEXT("/Game/Characters/Player/Axion_Step4F/SK_Player_Axion"));
    if (CharacterMesh.Succeeded())
    {
        GetMesh()->SetSkeletalMesh(CharacterMesh.Object);
        // Hide the fallback visual mesh to avoid double rendering
        if (FallbackVisualMesh)
        {
            FallbackVisualMesh->SetVisibility(false, false);
            FallbackVisualMesh->SetHiddenInGame(true);
        }
    }
    else
    {
        UE_LOG(LogTemp, Warning, TEXT("[AExcelionCharacter] Character skeletal mesh not found at /Game/Characters/Player/Axion_Step4F/SK_Player_Axion"));
    }
...
void AExcelionCharacter::PostInitializeComponents()
{
	...
	if (FallbackVisualMesh)
	{
		...
		if (bHasSkeletalMesh)
		{
			FallbackVisualMesh->SetVisibility(false, false);
			FallbackVisualMesh->SetHiddenInGame(true);
			UE_LOG(LogTemp, Warning, TEXT("[AExcelionCharacter] FallbackVisualMesh hidden because SkeletalMesh is valid (%s)"),
		...
		else
		{
			FallbackVisualMesh->SetVisibility(true, false);
			FallbackVisualMesh->SetHiddenInGame(false);
			UE_LOG(LogTemp, Warning, TEXT("[AExcelionCharacter] FallbackVisualMesh visible (SkeletalMesh is NULL)"));
		}
	}
...
void AExcelionCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		if (MoveAction)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("[INPUT] Bound IA_Move (Axis2D editor asset) - WASD Primary"));
		...
		if (LookAction)
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("[INPUT] Bound IA_Look (Axis2D editor asset)"));
		}
