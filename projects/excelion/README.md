# Excelion

Mission-Based 3D Action · 오리지널 메카 IP  
**Atlas 하위 개별 프로젝트** · SoR = **`projects/excelion/`** (이 트리)

> AI가 게임을 만드는 것이 아니라, 개발자가 더 좋은 게임을 만들도록 AI가 반복 작업을 대신한다.

**상태:** 스토리·설계 텍스트 진행 · 시각화·ParaModel = **HOLD** (Master 2026-08-04)  
**제작 파이프라인 (활성):** Meshy AI → Blender → FBX → UE  
(`state/MESHY_BLENDER_PIPELINE_SPEC.md` · TBD 유지)  
`ln9swrd/excelion-forge` = **DEPRECATION CANDIDATE** (비활성 · 외부 자산 보존 · 삭제 금지)

---

## Start here

| 순서 | Path | 역할 |
|------|------|------|
| 1 | [state/CURRENT_STATE.md](state/CURRENT_STATE.md) | 지금 · Next |
| 2 | [state/TASK_MAP.md](state/TASK_MAP.md) | 열린 작업 |
| 3 | [docs/09_STORY_S1.md](docs/09_STORY_S1.md) | S1 스토리 (13화 골격) |
| 4 | [novel/EP14_24_PLOT.md](novel/EP14_24_PLOT.md) | EP14–24 플롯 |
| 5 | [docs/README.md](docs/README.md) | 설계 인덱스 |
| 6 | [docs/08_PLAY_BRAVE.md](docs/08_PLAY_BRAVE.md) | 플레이 |

---

## Snapshot

| 항목 | 값 |
|------|-----|
| 장르 | Mission-Based 3D Action |
| 주인공 | **리아** (16세) |
| 성장 | **S-Core** |
| S1 | **24화 목표** |
| 최종 보스 | **네메시스** (여성형) |
| ELITE | 세스 (EP6) · 크레일 (EP15) |
| 방패 | 아이기스 (EP21) |
| 엔진 | Unreal · Blender |
| 파이프라인 | Meshy → Blender → FBX → UE |

---

## 최우선 원칙

- 분량에 상관없이 **재미 있는 소년만화**
- 스토리에 게임에 구현될 거의 모든 것이 담겨야 한다

---

## Layout

```
projects/excelion/
  README.md · PROJECT_CHARTER.md · PROJECT_MEMORY.md
  docs/           설계 SoR
  state/          운영 SoR
  design/         비주얼·에셋 스펙·제작 단위
  novel/          스토리 본문·바이블·플롯
  sprints/ · backlog.json   실행 (HOLD 시 동결)
```

## Related

| 위치 | 역할 |
|------|------|
| `projects/excelion/` | **게임 제품 SoR** (여기) |
| `ln9swrd/excelion` | 이전 단독 레포 · CLOSED |
| `ln9swrd/excelion-forge` | 구 파이프라인 · **DEPRECATION CANDIDATE** (보존) |
| atlas root | DevOS 플랫폼 · 하위 개별 HOLD |
