# ParaModel

Parametric mecha model system for Atlas / Excelion.

## Structure

```
ParaModel/
├── addon/                          # Blender addon (v0.1.0)
│   ├── __init__.py
│   ├── operators.py                # Load mecha / Clear slots
│   ├── panels.py                   # Sidebar UI
│   └── props.py                    # Scene settings
├── schema/
│   ├── base-body-slots.json        # Base Body 슬롯 정의
│   └── mecha-metadata.example.json
└── data/
    └── mecha/
        └── brave-001.json          # 첫 실제 데이터
```

## Status

- [x] Base Body slots definition
- [x] mecha-metadata example
- [x] First real data (brave-001)
- [x] Minimal Blender addon skeleton (v0.1.0)

## Blender Addon 설치

1. Blender → Edit → Preferences → Add-ons → Install
2. `ParaModel/addon` 폴더를 zip으로 묶어 설치하거나, scripts 경로에 복사
3. N 패널 → ParaModel 탭
4. Data Path에 `ParaModel/data/mecha` 경로 지정
5. Mecha ID 입력 후 Load Mecha

## Registered Units

| ID | Name | Codename | Status |
|----|------|----------|--------|
| brave-001 | Brave | EX-BRAVE-001 | concept |

## Notes

- BlenderMCP와 별개 프로젝트
- 현재: 슬롯 Empty 생성만 지원
- 다음: part_id 메시 attach, 파라미터 적용
