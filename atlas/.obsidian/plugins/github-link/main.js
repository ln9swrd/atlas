...
// src/keychain.ts
var storage;
function initKeychain(secretStorage) {
...
function setToken(account, token, secretName) {
...
function migrateTokens(accounts) {
...
var AccountSettings = class {
  ...
  renderNewAccount(container, saveNewAccountCallback) {
    ...
    new import_obsidian3.Setting(accountContainer).setName("Token").setDesc(
      ...
      button.onClick(async () => {
        var _a, _b;
        const clientId = ((_a = this.newAccount) == null ? void 0 : _a.customOAuth) ? this.newAccount.clientId : void 0;
        ...
        (_b = this.authModal) == null ? void 0 : _b.close();
        this.authModal = null;
      ...
      (el) => new import_obsidian3.SecretComponent(this.app, el).setValue(this.newAccount.tokenSecret).onChange((secretName) => {
        this.newAccount.tokenSecret = secretName;
        const token = this.app.secretStorage.getSecret(secretName);
        this.newAccount.token = token != null ? token : "";
      })
    );
  ...
  renderAccountSetting(account, parent = this.container) {
    ...
    new import_obsidian3.Setting(accountContainer).setName("Token").setDesc(
      ...
      (el) => new import_obsidian3.SecretComponent(this.app, el).setValue(account.tokenSecret).onChange(async (secretName) => {
        account.tokenSecret = secretName;
        const token = this.app.secretStorage.getSecret(secretName);
        account.token = token != null ? token : "";
        await this.saveCallback();
      })
    );
  }
...
var GithubLinkPluginSettingsTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(containerEl).setName("Show external link").setDesc("When using a custom query, an 'Open on GitHub' link can be added to view the results there.").addToggle((toggle) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Show refresh button").setDesc("Add a refresh button to tables to manually skip the cache.").addToggle((toggle) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Show pagination").setDesc("For query results with more than a single page of results, show pagination controls below the table.").addToggle((toggle) => {
    ...
    new import_obsidian4.Setting(containerEl).setClass("github-link-sub-setting").setName("Clear cache").setDesc("Seeing strange cache behavior? Clicking this will delete all cached responses.").addButton((button) => {
      ...
      button.onClick(async () => {
        const itemsDeleted = await getCache().clean(/* @__PURE__ */ new Date());
        await this.saveSettings();
...
function setPRIcon(icon, status) {
  ...
  (0, import_obsidian5.setIcon)(icon, PRIcon[status]);
...
function setPRMergeableIcon(icon, mergeable) {
  ...
  (0, import_obsidian5.setIcon)(icon, PRMergeableIcon[`${mergeable}`]);
...
function isEqual(value, other) {
  return (0, import_lodash.isEqual)(value, other);
}
...
function mapObject(value, config, removeUndefined = false, removeNull = false) {
  const resultEntries = [];
  for (const entry of Object.entries(config)) {
    const key = entry[0];
    const configSet = entry[1];
    if (configSet === true) {
      resultEntries.push([key, value[key]]);
    } else {
  ...
  if (removeUndefined || removeNull) {
    for (const [key, value2] of resultEntries) {
      if (removeUndefined && value2 === void 0 || removeNull && value2 === null) {
        continue;
      }
      finalEntries.push([key, value2]);
    }
  } else {
...
// src/query/sort.ts
function searchSortFromQuery(params) {
  if (params.sort !== "popularity" && params.sort !== "long-running") {
    return params.sort;
  }
  return void 0;
}
function issueListSortFromQuery(params) {
  if (params.sort && ["created", "updated", "comments"].includes(params.sort)) {
    return params.sort;
  }
  return void 0;
}
function pullListSortFromQuery(params) {
...
var _GitHubApi = class _GitHubApi {
  ...
  parseLinkHeader(link) {
    ...
    do {
      ...
      if (match == null ? void 0 : match.groups) {
        ...
        const page = parseInt((_a = params.get("page")) != null ? _a : "");
        const per_page = parseInt((_b = params.get("per_page")) != null ? _b : "");
        ...
        paginationMeta[match.groups.rel] = linkMeta;
      }
    } while (match);
    logger.debug(paginationMeta);
    return paginationMeta;
  }
  async getIssue(org, repo, issue, token, skipCache = false) {
    const { response } = await this.queueRequest(
      { url: `${_GitHubApi.baseApi}/repos/${org}/${repo}/issues/${issue}` },
      token,
      skipCache
    );
    return response.json;
  }
  async listIssuesForToken(params, token, skipCache = false) {
    const url = this.addParams(`${_GitHubApi.baseApi}/issues`, params);
    const { meta, response } = await this.queueRequest({ url }, token, skipCache);
    return { meta, response: response.json };
  }
  async listIssuesForRepo(org, repo, params = {}, token, skipCache = false) {
    const url = this.addParams(`${_GitHubApi.baseApi}/repos/${org}/${repo}/issues`, params);
    const { meta, response } = await this.queueRequest({ url }, token, skipCache);
    return { meta, response: response.json };
  }
  async listIssuesForOrganization(org, params, token, skipCache = false) {
    const url = this.addParams(`${_GitHubApi.baseApi}/orgs/${org}/issues`, params);
    const { meta, response } = await this.queueRequest({ url }, token, skipCache);
    return { meta, response: response.json };
  }
  async getPullRequest(org, repo, pr, token, skipCache = false) {
    const { response } = await this.queueRequest(
      {
  ...
  async listPullRequestsForRepo(org, repo, params = {}, token, skipCache = false) {
    const url = this.addParams(`${_GitHubApi.baseApi}/repos/${org}/${repo}/pulls`, params);
    const { meta, response } = await this.queueRequest({ url }, token, skipCache);
    return { meta, response: response.json };
  }
  async getCode(org, repo, path, branch, token, skipCache = false) {
    const { response } = await this.queueRequest(
      {
  ...
  async searchIssues(params, token, skipCache = false) {
    const url = this.addParams(`${_GitHubApi.baseApi}/search/issues`, params);
    const { meta, response } = await this.queueRequest({ url }, token, skipCache);
    return { meta, response: response.json };
  }
  async listCheckRunsForRef(org, repo, ref, token, skipCache = false) {
  ...
  async githubRequest(_config, token, skipCache = false) {
    ...
    try {
      ...
      logger.debug(config);
      const response = await (0, import_obsidian6.requestUrl)(config);
      ...
      logger.debug(err);
      return Promise.reject(new RequestError2(err));
    }
  }
  /**
   ...
   */
  cachedRequestIsRecent(cachedValue) {
    if (!cachedValue) {
      return false;
    }
...
function getIssue(org, repo, issue, skipCache = false) {
  return api.getIssue(org, repo, issue, getToken2(org), skipCache);
}
...
function getIssuesForRepo(params, org, repo, skipCache = false) {
  return api.listIssuesForRepo(org, repo, toIssueListParams(params), getToken2(org), skipCache);
}
function getIssuesForOrganization(params, org, skipCache = false) {
  const listParams = mapObject(
    params,
    {
      assignee: true,
      creator: true,
      direction: true,
      labels: (params2) => {
        if (Array.isArray(params2.labels)) {
          return params2.labels.join(",");
        }
        return params2.labels;
      },
      mentioned: true,
      page: true,
      per_page: true,
      since: true,
      sort: (params2) => issueListSortFromQuery(params2),
      state: true,
      filter: true
    },
    true,
  ...
  return api.listIssuesForOrganization(org, listParams, getToken2(org), skipCache);
}
function getPullRequest(org, repo, pullRequest, skipCache = false) {
  return api.getPullRequest(org, repo, pullRequest, getToken2(org), skipCache);
}
function getPullRequestsForRepo(params, org, repo, skipCache = false) {
  const listParams = mapObject(
    params,
    {
      direction: true,
      page: true,
      per_page: true,
      sort: (params2) => pullListSortFromQuery(params2),
      state: true
    },
    true,
    true
  ...
  setPageSize(listParams);
  return api.listPullRequestsForRepo(org, repo, listParams, getToken2(org), skipCache);
}
async function searchIssues(params, query, org, skipCache = false) {
  const searchParams = mapObject(
    params,
    {
      q: () => query,
      sort: (params2) => searchSortFromQuery(params2),
      order: (params2) => params2.order,
      page: (params2) => params2.page,
      per_page: (params2) => params2.per_page
    },
    true,
    true
  );
  setPageSize(searchParams);
  return api.searchIssues(searchParams, getToken2(org, query), skipCache);
}
async function getPRForIssue(timelineUrl, org) {
  var _a, _b, _c;
  let result = null;
  try {
    const { response } = await api.queueRequest({ url: timelineUrl }, getToken2(org));
...
function toIssueListParams(queryParams) {
  const listParams = mapObject(
    queryParams,
    {
      assignee: true,
...
function createIssueSection(config, parsedUrl, container) {
  if (parsedUrl.issue === void 0) {
    return;
  }
...
function createPullRequestSection(config, parsedUrl, container) {
...
function createErrorSection(config, container, error) {
  const errorIcon = createSpan({
    cls: ["github-link-inline-error-icon", "github-link-inline-icon"]
  });
  (0, import_obsidian7.setIcon)(errorIcon, "lucide-alert-triangle");
...
function InlineRenderer(el) {
  let githubLinks = Array.from(el.querySelectorAll(`a.external-link[href^="https://github.com"]`));
...
var InlineTagWidget = class _InlineTagWidget extends import_view.WidgetType {
  constructor(match) {
    super();
    this.match = match;
    this.href = match[0];
  ...
  eq(other) {
    return other instanceof _InlineTagWidget && other.href === this.href;
  }
  toDOM() {
...
function createInlineViewPlugin(_plugin) {
  class InlineViewPluginValue {
    ...
    isLivePreview() {
      return this.view.state.field(import_obsidian8.editorLivePreviewField);
    }
    /**
     * Check if the decoration at the given position, with the given match, should render
     */
    shouldRender(decorationFrom, decorationTo, match) {
      var _a, _b;
  ...
  return import_view.ViewPlugin.fromClass(InlineViewPluginValue, {
...
// src/github/cache.ts
var DB_NAME = "obsidian-github-link";
var STORE_NAME = "request-cache";
var DB_VERSION = 1;
...
var RequestCache = class {
  ...
  get(request2) {
    var _a;
    const entry = (_a = this.memCache[this.getCacheKey(request2)]) != null ? _a : null;
    if (entry && !entry.response.headers) {
      entry.response.headers = {};
    }
    return entry;
  }
  async set(request2, response) {
    var _a, _b, _c;
    ...
    const key = this.getCacheKey(request2);
    this.memCache[key] = entry;
    await ((_c = this.db) == null ? void 0 : _c.put(STORE_NAME, entry.toStored()));
  }
  async remove(request2) {
    var _a;
    const key = typeof request2 === "string" ? request2 : this.getCacheKey(request2);
    delete this.memCache[key];
    await ((_a = this.db) == null ? void 0 : _a.delete(STORE_NAME, key));
  }
  async clean(maxAge) {
    var _a;
  ...
  async update(request2) {
    ...
    if (entry) {
      ...
      await ((_a = this.db) == null ? void 0 : _a.put(STORE_NAME, entry.toStored()));
    }
  }
  /**
   * Import entries from the old JSON-based cache (data.json migration).
   */
  async importFromJSON(storedCache) {
    var _a;
    let imported = 0;
...
function UserCell(user, el) {
  var _a;
  const anchor = el.createEl("a", {
    cls: "github-link-table-author",
    href: (_a = user == null ? void 0 : user.html_url) != null ? _a : "#",
    attr: { target: "_blank" }
  });
  if (user == null ? void 0 : user.avatar_url) {
    anchor.createEl("img", { cls: "github-link-table-avatar", attr: { src: user.avatar_url } });
  }
  anchor.createSpan({ text: user == null ? void 0 : user.login });
}
var CommonIssuePRColumns = {
  ...
  repo: {
    ...
    cell: (row, el) => {
      ...
      el.createEl("a", { text: (_a = parsed == null ? void 0 : parsed.repo) != null ? _a : "Repo", href: url, attr: { target: "_blank" } });
    }
  },
  author: {
    header: "Author",
  ...
  labels: {
    ...
    cell: (row, el) => {
      ...
      for (const label of (_a = row.labels) != null ? _a : []) {
...
var IssueColumns = {
  ...
  status: {
    ...
    cell: (row, el) => {
      ...
      wrapper.createSpan({
        text: row.state_reason === "not_planned" ? "Not Planned" : titleCase(status)
      });
    }
  },
  pr: {
...
var DEFAULT_COLUMNS = {
  ["issue"]: ["number", "title", "author", "created", "status"],
  ["pull-request"]: ["number", "title", "author", "created", "status"],
  ["repo"]: []
};
var ALL_COLUMNS = {
  ["issue"]: IssueColumns,
  ["pull-request"]: PullRequestColumns,
  ["repo"]: RepoColumns
};

// src/query/query.ts
var GithubQuery = class {
   ...
   */
  async setParams(newParams = this.params, forceUpdate = false) {
  ...
  parseCodeblock(source) {
    let params;
    try {
      params = (0, import_obsidian9.parseYaml)(source);
    } catch (e) {
  ...
  async executeQuery(skipCache = false) {
    const params = this.params;
    if (params.outputType === "table" /* Table */) {
      if (params.query && (params.queryType === "issue" /* Issue */ || params.queryType === "pull-request" /* PullRequest */)) {
        const { meta, response } = await searchIssues(params, params.query, params.org, skipCache);
        return { meta, response: response.items };
      } else if (params.queryType === "issue" /* Issue */ && params.org && params.repo) {
        return await getIssuesForRepo(params, params.org, params.repo, skipCache);
      } else if (params.queryType === "issue" /* Issue */ && params.org) {
        return await getIssuesForOrganization(params, params.org, skipCache);
      } else if (params.queryType === "issue" /* Issue */) {
        return await getMyIssues(params, params.org, skipCache);
      } else if (params.queryType === "pull-request" /* PullRequest */ && params.org && params.repo) {
  ...
  renderHeader(table, queryType, columns) {
    var _a, _b;
    const thead = table.createEl("thead");
    for (const col of columns) {
      const th = thead.createEl("th");
      th.setText((_b = (_a = ALL_COLUMNS[queryType][col]) == null ? void 0 : _a.header) != null ? _b : titleCase(col));
    }
  }
  renderBody(table, queryType, columns, result) {
  ...
  renderCell(tr, queryType, column, row) {
  ...
  renderFooter(params, meta, parent) {
  ...
  renderPagination(meta, parent) {
    var _a;
    if (PluginSettings.showPagination && this.hasSomeRel(meta)) {
      const pagination = parent.createDiv({ cls: "github-link-table-pagination" });
      if ((meta == null ? void 0 : meta.first) && (!meta.prev || meta.prev.page !== meta.first.page)) {
        const first = pagination.createEl("a", { text: "<<", href: "#", attr: { role: "button" } });
        first.addEventListener("click", () => {
          var _a2;
          void this.setParams({ ...this.params, page: (_a2 = meta.first) == null ? void 0 : _a2.page });
        });
      }
      if (meta == null ? void 0 : meta.prev) {
        const prev = pagination.createEl("a", { text: meta.prev.page.toString(), href: "#", attr: { role: "button" } });
        prev.addEventListener("click", () => {
          var _a2;
          void this.setParams({ ...this.params, page: (_a2 = meta.prev) == null ? void 0 : _a2.page });
        });
      }
      pagination.createSpan({ text: ((_a = this.params.page) != null ? _a : 1).toString() });
      if (meta == null ? void 0 : meta.next) {
        const next = pagination.createEl("a", { text: meta.next.page.toString(), href: "#", attr: { role: "button" } });
        next.addEventListener("click", () => {
          var _a2;
          void this.setParams({ ...this.params, page: (_a2 = meta.next) == null ? void 0 : _a2.page });
        });
      }
      if ((meta == null ? void 0 : meta.last) && (!meta.next || meta.next.page !== meta.last.page)) {
        const last = pagination.createEl("a", { text: ">>", href: "#", attr: { role: "button" } });
        last.addEventListener("click", () => {
          var _a2;
          void this.setParams({ ...this.params, page: (_a2 = meta.last) == null ? void 0 : _a2.page });
        });
      }
    }
  }
...
var PluginData = { settings: PluginSettings, dataVersion: DATA_VERSION };
